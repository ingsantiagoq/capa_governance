...
test('UBP registry promotes AP, Control Plane, Ledger and Tax after complete evidence', async () => {
  ...
  assert.deepEqual(active, ['ap-expert', 'audit-integrity-expert', 'control-plane-expert', 'identity-access-expert', 'ledger-expert', 'tax-expert']);
...
test('committed UBP seed audit remains tied to current manifests and registry revision', async () => {
  ...
  const manifests = [apExpert, ar, auditIntegrityExpert, controlPlaneExpert, identityAccessExpert, inventoryExpert, ledgerExpert, taxExpert];
  const verifiedSeeds = new Set(['ap-expert', 'audit-integrity-expert', 'control-plane-expert', 'identity-access-expert', 'ledger-expert', 'tax-expert']);
...
test('PO governance publishes coherent AP, Audit, Control Plane, Identity, Ledger and Tax authorities', async () => {
  ...
  assert.equal(input.expertManifests.length, 6);
  assert.deepEqual(input.seedAudit.experts.map(item => item.expertId).sort(), ['ap-expert', 'audit-integrity-expert', 'control-plane-expert', 'identity-access-expert', 'ledger-expert', 'tax-expert']);
  const result = evaluatePoGovernance(input);
  assert.equal(result.decision, 'CONTROLLED');
  assert.deepEqual(result.summary, { ready: 6, blocked: 24, total: 30 });
