...
const auditIntegrityExpert = JSON.parse(await readFile(new URL('../catalog/ubp/experts/audit-integrity.expert.manifest.json', import.meta.url), 'utf8'));
const documentManagementExpert = JSON.parse(await readFile(new URL('../catalog/ubp/experts/document-management.expert.manifest.json', import.meta.url), 'utf8'));
...
const fixedAssetsExpert = JSON.parse(await readFile(new URL('../catalog/ubp/experts/fixed-assets.expert.manifest.json', import.meta.url), 'utf8'));
...
test('UBP registry publishes fifteen authorities only with complete evidence', async () => {
  ...
  assert.deepEqual(active, ['ap-expert', 'audit-integrity-expert', 'control-plane-expert', 'document-management-expert', 'electronic-fiscal-documents-expert', 'fixed-assets-expert', 'fx-expert', 'identity-access-expert', 'inventory-expert', 'ledger-expert', 'legal-numbering-expert', 'localization-country-pack-expert', 'tax-expert', 'tenant-organization-expert', 'treasury-expert']);
...
test('committed UBP seed audit remains tied to current manifests and registry revision', async () => {
  ...
  const manifests = [apExpert, ar, auditIntegrityExpert, controlPlaneExpert, documentManagementExpert, electronicFiscalDocumentsExpert, fixedAssetsExpert, fxExpert, identityAccessExpert, inventoryExpert, ledgerExpert, legalNumberingExpert, localizationCountryPackExpert, taxExpert, tenantOrganizationExpert, treasuryExpert];
...
test('PO governance publishes fifteen coherent authorities', async () => {
  ...
  assert.equal(input.expertManifests.length, 15);
  ...
  assert.deepEqual(result.summary, { ready: 15, blocked: 15, total: 30 });
  ...
  assert.equal(result.experts.find(item => item.expertId === 'document-management-expert').decision, 'READY');
  assert.equal(result.experts.find(item => item.expertId === 'electronic-fiscal-documents-expert').decision, 'READY');
  assert.equal(result.experts.find(item => item.expertId === 'fixed-assets-expert').decision, 'READY');
...
test('Localization & Country Pack separates publication from country readiness', async () => {
  const base = JSON.parse(await readFile(new URL('../examples/tax.po-request.json', import.meta.url), 'utf8'));
  const catalog = await loadPoCatalog(base.evaluatedAt);
  const governed = structuredClone(base);
  governed.request = { ...governed.request, domain: 'localization-country-pack', capability: 'localization-country-pack', action: 'inspect-pack-provenance', impactedDomains: ['localization-country-pack'] };
  const ready = evaluateProductDecision(governed, catalog);
  assert.equal(ready.decision, 'GO');
  assert.equal(ready.primaryExpert, 'localization-country-pack-expert');

  governed.request.action = 'declare-country-supported';
...
test('Fixed Assets governs evidenced depreciation and blocks unsupported books and inventory depreciation', async () => {
  const base = JSON.parse(await readFile(new URL('../examples/tax.po-request.json', import.meta.url), 'utf8'));
  const catalog = await loadPoCatalog(base.evaluatedAt);
  const governed = structuredClone(base);
  governed.request = { ...governed.request, domain: 'fixed-assets', capability: 'fixed-assets', action: 'inspect-depreciation-schedule', impactedDomains: ['fixed-assets'] };
  const ready = evaluateProductDecision(governed, catalog);
  assert.equal(ready.decision, 'GO');
  assert.equal(ready.primaryExpert, 'fixed-assets-expert');

  governed.request.action = 'declare-multi-book-ready';
  const unsupported = evaluateProductDecision(governed, catalog);
  assert.equal(unsupported.decision, 'BLOCK');
  assert.equal(unsupported.stage, 'authority-readiness');
...
test('Document Management governs evidence while repository and destructive claims remain blocked', async () => {
  const base = JSON.parse(await readFile(new URL('../examples/tax.po-request.json', import.meta.url), 'utf8'));
  const catalog = await loadPoCatalog(base.evaluatedAt);
  const governed = structuredClone(base);
  governed.request = {...governed.request, domain: 'document-management', capability: 'document-management', action: 'inspect-document-foundations', impactedDomains: ['document-management'] };
  const ready = evaluateProductDecision(governed, catalog);
  assert.equal(ready.decision, 'GO');
  assert.equal(ready.primaryExpert, 'document-management-expert');

  governed.request.action = 'declare-document-management-runtime-ready';
  const unsupported = evaluateProductDecision(governed, catalog);
  assert.equal(unsupported.decision, 'BLOCK');
  assert.equal(unsupported.stage, 'authority-readiness');

  governed.request.action = 'mutate-immutable-document-version';
  const forbidden = evaluateProductDecision(governed, catalog);
  assert.equal(forbidden.decision, 'BLOCK');
  assert.deepEqual(forbidden.reasons, ['expert-restriction']);
});
...
test('FX governs product direction while proposed ADR and runtime claims remain blocked', async () => {
  const base = JSON.parse(await readFile(new URL('../examples/tax.po-request.json', import.meta.url), 'utf8'));
  const catalog = await loadPoCatalog(base.evaluatedAt);
  const governed = structuredClone(base);
  governed.request = { ...governed.request, domain: 'fx', capability: 'fx', action: 'inspect-fx-foundations', impactedDomains: ['fx'] };
  const ready = evaluateProductDecision(governed, catalog);
  assert.equal(ready.decision, 'GO');
  assert.equal(ready.primaryExpert, 'fx-expert');

  governed.request.action = 'declare-fx-runtime-ready';
  const unsupported = evaluateProductDecision(governed, catalog);
  assert.equal(unsupported.decision, 'BLOCK');
  assert.equal(unsupported.stage, 'authority-readiness');

  governed.request.action = 'reuse-current-rate-for-historical-fact';
  const forbidden = evaluateProductDecision(governed, catalog);
  assert.equal(forbidden.decision, 'BLOCK');
  assert.deepEqual(forbidden.reasons, ['expert-restriction']);
});

test('Electronic Fiscal Documents governs provider orchestration and blocks false acceptance and E2E claims', async () => {
  const base = JSON.parse(await readFile(new URL('../examples/tax.po-request.json', import.meta.url), 'utf8'));
  const catalog = await loadPoCatalog(base.evaluatedAt);
  const governed = structuredClone(base);
  governed.request = { ...governed.request, domain: 'electronic-fiscal-documents', capability: 'electronic-fiscal-documents', action: 'inspect-fiscal-document-provenance', impactedDomains: ['electronic-fiscal-documents'] };
  const ready = evaluateProductDecision(governed, catalog);
  assert.equal(ready.decision, 'GO');
  assert.equal(ready.primaryExpert, 'electronic-fiscal-documents-expert');

  governed.request.action = 'declare-electronic-fiscal-documents-e2e';
  const unsupported = evaluateProductDecision(governed, catalog);
  assert.equal(unsupported.decision, 'BLOCK');
  assert.equal(unsupported.stage, 'authority-readiness');

  governed.request.action = 'mark-accepted-without-fiscal-identifier';
  const forbidden = evaluateProductDecision(governed, catalog);
  assert.equal(forbidden.decision, 'BLOCK');
  assert.deepEqual(forbidden.reasons, ['expert-restriction']);
});

test('Legal Numbering governs issuance and blocks unproven sovereignty and history mutation', async () => {
  const base = JSON.parse(await readFile(new URL('../examples/tax.po-request.json', import.meta.url), 'utf8'));
  const catalog = await loadPoCatalog(base.evaluatedAt);
  const governed = structuredClone(base);
  governed.request = { ...governed.request, domain: 'legal-numbering', capability: 'legal-numbering', action: 'inspect-numbering-provenance', impactedDomains: ['legal-numbering'] };
  const ready = evaluateProductDecision(governed, catalog);
  assert.equal(ready.decision, 'GO');
  assert.equal(ready.primaryExpert, 'legal-numbering-expert');

  governed.request.action = 'declare-legal-numbering-sovereign';
  const unsupported = evaluateProductDecision(governed, catalog);
  assert.equal(unsupported.decision, 'BLOCK');
  assert.equal(unsupported.stage, 'authority-readiness');

  governed.request.action = 'reuse-consumed-legal-number';
  const forbidden = evaluateProductDecision(governed, catalog);
  assert.equal(forbidden.decision, 'BLOCK');
  assert.deepEqual(forbidden.reasons, ['expert-restriction']);
});

test('Tenant & Organization enforces the superseding company boundary', async () => {
  const base = JSON.parse(await readFile(new URL('../examples/tax.po-request.json', import.meta.url), 'utf8'));
  const catalog = await loadPoCatalog(base.evaluatedAt);
  const governed = structuredClone(base);
  governed.request = { ...governed.request, domain: 'tenant-organization', capability: 'tenant-organization', action: 'inspect-tenant-ownership', impactedDomains: ['tenant-organization'] };
  const ready = evaluateProductDecision(governed, catalog);
  assert.equal(ready.decision, 'GO');
  assert.equal(ready.primaryExpert, 'tenant-organization-expert');

  governed.request.action = 'declare-tenant-organization-migration-complete';
  const blocked = evaluateProductDecision(governed, catalog);
  assert.equal(blocked.decision, 'BLOCK');
  assert.equal(blocked.stage, 'authority-readiness');

  governed.request.action = 'expose-legal-entity-selector';
  const forbidden = evaluateProductDecision(governed, catalog);
  assert.equal(forbidden.decision, 'BLOCK');
  assert.deepEqual(forbidden.reasons, ['expert-restriction']);
});

test('Audit & Integrity governs inspection and blocks unproven completeness claims', async () => {
  const base = JSON.parse(await readFile(new URL('../examples/tax.po-request.json', import.meta.url), 'utf8'));
  const catalog = await loadPoCatalog(base.evaluatedAt);
  const governed = structuredClone(base);
  governed.request = { ...governed.request, domain: 'audit-integrity', capability: 'audit-integrity', action: 'inspect-functional-audit', impactedDomains: ['audit-integrity'] };
  const ready = evaluateProductDecision(governed, catalog);
  assert.equal(ready.decision, 'GO');
  assert.equal(ready.primaryExpert, 'audit-integrity-expert');

  governed.request.action = 'declare-cross-service-audit-coverage-complete';
  const blocked = evaluateProductDecision(governed, catalog);
  assert.equal(blocked.decision, 'BLOCK');
  assert.equal(blocked.stage, 'authority-readiness');
});
